...
class Bakery:
    ...
    @name.setter
    def name(self, value):
        if not value or value.isspace():   # if value.strip() == ""
    ...
    def add_drink(self, drink_type: str, name: str, portion: float, brand: str):
        ...
        if drink_type in ["Tea", "Water"]:
            ...
            if current_drink:
                raise Exception(f"{drink_type} {current_drink.name} is already in the menu!")
            if drink_type == "Tea":
    ...
    def reserve_table(self, number_of_people: int):
        free_table = next(filter(lambda x: x.is_reserved is False and x.capacity >= number_of_people, self.tables_repository), None)
        if not free_table:
    ...
    def order_food(self, table_number: int, *food_names):
        ...
        display_list = [f'Table {table_number} ordered:', *foods_in_menu,
                        f'{self.name} does not have in the menu:', *foods_not_in_menu]
        return "\n".join(display_list)
    ...
    def order_drink(self, table_number: int, *drink_names):
        ...
        for drink_name in drink_names:
            name = self.find_drink(drink_name)
            if name:
                drinks_in_menu.append(name.__repr__())
                current_table.drink_orders.append(name)
            else:
    ...
    def leave_table(self, table_number: int):
        ...
        output.append(f"Bill: {table_bill:.2f}")
        current_table.clear()
        return '\n'.join(output)
